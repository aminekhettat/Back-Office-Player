...
All coverage is provided by the main test suite; these remain as
a readable integration smoke-check.
...
def test_audio_player_basics():
    ...
    assert player.get_tempo() == 0.5
    player.set_tempo(5.0)
    assert player.get_tempo() == 2.0


@pytest.mark.integration
def test_segments():
    """Segment creation, SegmentManager operations, serialisation."""
...
def test_persistence():
    """Segment save/load cycle via filesystem."""
...
def test_config_export_import():
    """Configuration export/import round-trip via .bop JSON files."""
    with tempfile.TemporaryDirectory() as tmpdir:
